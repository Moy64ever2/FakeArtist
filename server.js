...
setInterval(() => {
  const now = Date.now();
  const DISCONNECT_TIMEOUT = 45 * 1000; // 45 seconds (3 missed heartbeats)
    ...
    if (now - heartbeat.lastSeen > DISCONNECT_TIMEOUT) {
      ...
      if (room) {
        ...
        if (heartbeat.isHost) {
          ...
          console.log('👤 Regular player disconnected, removing from room:', {
            playerId,
            playerName,
            roomId: heartbeat.roomId
          });
          ...
          if (room.gamePhase === 'drawing' && room.players[room.currentTurn]?.id === playerId) {
            ...
            console.log('🔄 Adjusted current turn after disconnect:', {
              ...
              newCurrentPlayer: room.players[room.currentTurn]?.name
            });
          }
        }
      }
...
}, 20000); // Check every 20 seconds for faster disconnect detection
...
app.post('/api/game/:roomId/heartbeat', (req, res) => {
  ...
  try {
    const room = gameState.rooms[roomId];
    if (!room) {
      console.log('❌ Heartbeat: Room not found:', roomId);
      return res.status(404).json({ error: 'Room not found' });
    }
    
    const player = room.players.find(p => p.id === playerId);
    if (!player) {
      console.log('❌ Heartbeat: Player not found:', { playerId, roomId });
    ...
    if (!wasTracked) {
      console.log('💓 New heartbeat tracking started:', {
        playerId,
