...
// Clean up disconnected players every 2 minutes
setInterval(() => {
  const now = Date.now();
  ...
  for (const [playerId, heartbeat] of playerHeartbeats.entries()) {
    if (now - heartbeat.lastSeen > DISCONNECT_TIMEOUT) {
      console.log('🔌 Player disconnected due to timeout:', playerId);
      
      const room = gameState.rooms[heartbeat.roomId];
      if (room) {
        if (heartbeat.isHost) {
          console.log('🏠 Host disconnected, closing room:', heartbeat.roomId);
          // Close the room
          delete gameState.rooms[heartbeat.roomId];
        } else {
          console.log('👤 Regular player disconnected, removing from room:', playerId);
...
app.post('/api/game/:roomId/join', (req, res) => {
  ...
  const { playerId, playerName, avatar = '😀', color = '#FF6B6B' } = req.body;
  
  console.log('Player joining room:', { roomId, playerId, playerName, avatar, color });
