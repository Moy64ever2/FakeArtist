...
setInterval(() => {
  const now = Date.now();
  const DISCONNECT_TIMEOUT = 60 * 1000; // 60 seconds (6 missed heartbeats at 10s intervals)
  ...
  for (const [playerId, heartbeat] of playerHeartbeats.entries()) {
    if (now - heartbeat.lastSeen > DISCONNECT_TIMEOUT) {
      ...
      if (room) {
        ...
        if (heartbeat.isHost) {
          console.log('🏠 Host disconnected due to timeout, closing room:', {
            roomId: heartbeat.roomId,
            hostName: playerName
          });
          // Close the room
...
console.log('✅ Server initialization complete'); server.close(() => {
    console.log('Server closed');
    process.exit(0);
  });
});
